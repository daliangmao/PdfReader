...
public class MuPDFCore
{
	...
	public synchronized void drawPage(Bitmap bitmap, int page,
			int pageW, int pageH,
			int patchX, int patchY,
			int patchW, int patchH,
			Cookie cookie) {
        ...
        if(mNightMode)
        {
            ...
            if(mNightCahce == null || curWidth != bitmap.getWidth() || curHeight != bitmap.getHeight())
            {
                mNightCahce = Bitmap.createBitmap(bitmap.getWidth(), bitmap.getHeight(), Bitmap.Config.ARGB_8888);
                curWidth = bitmap.getWidth();
                curHeight = bitmap.getHeight();
            }
            drawPage(mNightCahce, pageW, pageH, patchX, patchY, patchW, patchH, cookie.cookiePtr);
            Canvas board = new Canvas(bitmap);
            setInvert(cm);
            paint.setColorFilter(new ColorMatrixColorFilter(cm));
            Rect src = new Rect(patchX, patchY, patchX + patchW, patchY + patchH);
            board.drawBitmap(mNightCahce, src, src, paint);
        }
        else
		    drawPage(bitmap, pageW, pageH, patchX, patchY, patchW, patchH, cookie.cookiePtr);
	}

    private static void setInvert(ColorMatrix cm)
    {
        cm.set(new float[] {
