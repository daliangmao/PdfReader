...
import com.artifex.mupdfdemo.ReaderView.ReaderViewDelegate;
...
public class MuPDFActivity extends RightDrawerActivity implements FilePicker.FilePickerSupport, ReaderViewDelegate
{
    private final int SETTING_REQUEST = 100;
    private final int BOOKMARK_REQUEST = 101;

    @Override
    public void pageChange(int mCurrent) {
        this.setResult(mCurrent+1);
        mBookmark.setSelected(bookmarks.get(mCurrent)!=null);
    }
    ...
    private String mTitle, mBookid, mUser;
    private Boolean mPreview;
    private ImageButton mSetting;
    private ImageButton mBookmark;
    private ImageButton mBookmarkTable;
    private Map<Integer, String> bookmarks;
    //private ImageButton mTableContent;
	...
	private MuPDFCore openFile(String path)
	{
		...
		try
		{
            ...
            core.setNightMode(configure.confg.theme==1);
	...
	public void onCreate(Bundle savedInstanceState)
	{
        ...
        Configure.getInstance(this);
        mTitle = bundle.getString("title");
        mPreview = bundle.getBoolean("preview");
        mUser = bundle.getString("user");
        mBookid = bundle.getString("book");
	...
	public void createUI(Bundle savedInstanceState) {
        ...
        */
		...
		mDocView.setDisplayedViewIndex(mPreview?0:prefs.getInt("page"+mFileName, 0));
		...
		*/
        ...
        frame.addView(mDocView);
        frame.addView(mButtonsView);
	}
		...
		switch (requestCode) {
            ...
            case BOOKMARK_REQUEST:
                if (resultCode == 1) {
                    ...
                    Integer page = bundle.getInt("page");
                    if (mDocView.getDisplayedViewIndex() != page)
                        mDocView.setDisplayedViewIndex(page);
                    initBookmark();
                    mBookmark.setSelected(bookmarks.get(mDocView.getDisplayedViewIndex())!=null);
                }
                break;
            case SETTING_REQUEST:
                ...
                if (resultCode == 1) {
                    Bundle bundle = data.getExtras();
                    Integer theme = bundle.getInt("theme");
                    core.setNightMode(theme == 1);
                    mDocView.setDisplayedViewIndex(mDocView.getDisplayedViewIndex());
                }
                */
                Configure configure = Configure.getInstance();
                core.setNightMode(configure.confg.theme==1);
    ...
    */
	...
	private void makeButtonsView() {
        ...
        mSetting = (ImageButton)mButtonsView.findViewById(R.id.btnSetting);
        mBookmark = (ImageButton)mButtonsView.findViewById(R.id.btnBookmark);
        mBookmarkTable = (ImageButton)mButtonsView.findViewById(R.id.btnBookmarkTable);
        //mTableContent = (ImageButton)mButtonsView.findViewById(R.id.btnContent);
        if (mPreview) {
            //mTableContent.setVisibility(View.GONE);
            mBookmarkTable.setVisibility(View.GONE);
            mBookmark.setVisibility(View.GONE);
            RelativeLayout.LayoutParams params = (RelativeLayout.LayoutParams) mSetting.getLayoutParams();
            params.setMargins(0, 0, 10, 0);
            mSetting.setLayoutParams(params);
        }
    ...
    public void OnSettingButtonClick(View v) {
        Intent i = new Intent(MuPDFActivity.this, SettingActivity.class);
        startActivityForResult(i, SETTING_REQUEST);
        overridePendingTransition(R.anim.fade_in, R.anim.fade_out);
    }

    public void OnBookmarkButtonClick(View v) {
        Configure configure = Configure.getInstance();
        Integer page = mDocView.getDisplayedViewIndex();
        Cursor cursor = configure.bookmark.getItem(mBookid, mUser, page);
        if (cursor != null) {
            configure.bookmark.Delete(cursor.getInt(cursor.getColumnIndexOrThrow("mark_id")));
            bookmarks.remove(page);
            mBookmark.setSelected(false);
        }
        else {
            configure.bookmark.Add(mBookid, mUser, page);
            bookmarks.put(page, "Untitled");
            mBookmark.setSelected(true);
            Intent i = new Intent(MuPDFActivity.this, BookmarkTableActivity.class);
    ...
    public void OnBookmarkTableButtonClick(View v) {
        //mDrawerLayout.openDrawer(mRightDrawerView);
        Intent i = new Intent(MuPDFActivity.this, BookmarkTableActivity.class);
        Integer page = 0;//mDocView.getDisplayedViewIndex();
        Bundle bundle = new Bundle();
        bundle.putString("user", mUser);
        bundle.putString("bookid", mBookid);
        bundle.putInt("page", page);
    ...
    public void OnContentButtonClick(View v) {
        ...
        TableContentViewController tableContentViewController = new TableContentViewController(this);
        TextView txttitle = new TextView(this);
        txttitle.setText("Hello");
        txttitle.setTextColor(Color.DKGRAY);
        //mRightDrawerView.addView(txttitle);
    }
    ...
    private void initBookmark() {
        Configure configure = Configure.getInstance();
        Cursor curBookmark = configure.bookmark.All(mBookid, mUser);
        bookmarks.clear();
        if (curBookmark != null) {
            do {
                Integer page = curBookmark.getInt(curBookmark.getColumnIndexOrThrow("page"));
                String title = curBookmark.getString(curBookmark.getColumnIndexOrThrow("title"));
                bookmarks.put(page, title);
            }while(curBookmark.moveToNext());
        }
    }
